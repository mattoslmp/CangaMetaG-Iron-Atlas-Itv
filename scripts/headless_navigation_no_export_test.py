...
def run_one(page_index: int) -> int:
  # Headless navigation validates page construction, not static image export.
  # Disable Kaleido calls so every page can be exercised quickly and offline.
  try:
    from plotly.basedatatypes import BaseFigure
    BaseFigure.to_image = lambda self, *args, **kwargs: None
  except Exception:
    pass
  ...
  os.environ.setdefault("CANGAMETAG_DISABLE_STATIC_EXPORT", "1")
  os.chdir(ROOT)
...
def main() -> int:
  ...
  target = ROOT / "validation" / "LATEST_APP_HEADLESS_NAVIGATION_TEST.tsv"
  out.to_csv(target, sep="\t", index=False)
  if (out["status"] != "PASS").any():
    print(out[out["status"] != "PASS"].to_string(index=False))
    return 1
  print("LATEST_APP_HEADLESS_NAVIGATION_PASS")
  return 0
