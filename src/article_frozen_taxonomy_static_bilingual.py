...
"""Generate bilingual static Figures 4/5 from identical frozen values."""
...
CACHE_VERSION = "frozen_article_taxonomy_static_bilingual_v5"
...
def materialize_frozen_article_static_bilingual(
  domain: str,
  ...
  extent = max(
    float(np.max(np.abs(sites[["RDA1", "RDA2"]].to_numpy(float)))),
    1e-6,
  )
