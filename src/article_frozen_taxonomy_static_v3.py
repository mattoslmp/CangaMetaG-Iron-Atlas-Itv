...
"""Generate final Figure 4/5 SVGs with all legends below the four panels.

All scientific values are loaded from the frozen article tables. This module
changes layout only: abundance values, NMDS coordinates/stress, RDA scores,
vectors and statistics are not recomputed.
"""
...
def materialize_frozen_article_static_v3(domain: str, runtime_root: Path | str) -> Path:
  ...
  target_dir = Path(runtime_root) / CACHE_VERSION
  ...
  ax_c.set_title(
    f"C  Bray-Curtis NMDS (stress = {float(stats['NMDS_stress']):.3f})",
    loc="left",
    fontsize=20,
    fontweight="bold",
    pad=10,
  ...
  ax_d.set_title(
    f"D  RDA biplot (R² = {float(stats['RDA_R2']):.2f}; P = {float(stats['RDA_p']):.3f})",
    loc="left",
    fontsize=20,
    fontweight="bold",
    pad=10,
  ...
  lake_season_handles = [
    ...
    Line2D(
      ...
      linestyle="None", color="black", label=season, markersize=9,
    )
    for season in ["Dry", "Rainy"]
  ]
  rda_handles = [
    Line2D([0], [0], color="#333333", lw=1.8, label="Environmental variable"),
    Line2D([0], [0], color="#666666", lw=1.8, linestyle="--", label="Representative genus vector"),
  ]
  genus_handles = [
    Patch(
  ...
  fig.suptitle(
    f"{canonical} genus-level taxonomic profiles and ordination",
    fontsize=25,
    fontweight="bold",
    y=0.985,
  )
