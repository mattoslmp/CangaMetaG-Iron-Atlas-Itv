...
Deployment configuration and pre-deployment checks are documented in [`STREAMLIT_COMMUNITY_CLOUD.md`](STREAMLIT_COMMUNITY_CLOUD.md). The app uses Python 3.12, the root `requirements.txt`, `.streamlit/config.toml`, and repository-relative data paths.

<!-- PRIVATE_STREAMLIT_REVIEW_SECTIONS -->

## Interactive application

`app.py` is the Streamlit entrypoint. The private review build preserves the validated scientific matrices, sample identifiers, filters, axes, figures and interactive analyses. It includes taxonomic profiles, diversity and ordination analyses, IMG/JGI functional annotations, ST8 comparisons, iron-marker analyses, KEGG/KEMET module completeness and the final figure/script catalogue.

## Installation

Python 3.12 is the validated deployment target.
...
Windows users may also run `run_app_windows.bat` after installing the requirements.

## Institutional research provenance

This interactive application and the study-specific results presented here were developed by **Leandro de Mattos Pereira** during his postdoctoral research at the **Instituto Tecnológico Vale (ITV)**, under the supervision of **Dr. Gisele Lopes Nunes**.

Este aplicativo interativo e os resultados específicos apresentados foram desenvolvidos por **Leandro de Mattos Pereira** durante seu pós-doutorado no **Instituto Tecnológico Vale (ITV)**, sob a supervisão da **Dra. Gisele Lopes Nunes**.

This statement records research provenance only. It does not claim commercial endorsement, funding, intellectual-property ownership or public-release authorization by ITV or Dr. Gisele Lopes Nunes. No ITV logo is included in this private-review preparation because no separate authorized logo file was supplied for this step.

## Runtime data

The files needed to open the validated pages and reproduce the documented analyses are listed in `deployment/runtime_files_manifest.tsv`. Files intentionally withheld from the private repository are listed in `deployment/excluded_files_manifest.tsv`. Runtime paths are resolved from the repository root with `pathlib.Path`; optional user state is stored outside the repository through `src/runtime_paths.py`.

## Git LFS

The release policy is documented in `deployment/git_lfs_manifest.tsv`. After excluding manuscripts, editorial supplements, the 251 MB TIFF and raw archives that are not required at runtime, no included individual runtime file reaches 50 MiB. Therefore, this release does not add a new LFS object. Future required files at or above 100 MiB must use Git LFS, and required binary files at or above 50 MiB should preferentially use Git LFS with exact path rules.

## Security and secrets

Real credentials must never be committed. Use `.streamlit/secrets.example.toml` only as a variable-name template. Keep `.streamlit/secrets.toml`, `.env`, tokens, passwords, API credentials and private keys outside Git. Potential optional secrets are documented in `STREAMLIT_DEPLOYMENT.md`.

## Future Streamlit deployment

Deployment preparation is documented in `STREAMLIT_DEPLOYMENT.md`. Streamlit deployment has **not** been activated. No public application URL exists.
...
Public application URL: not available
Databiomics page: not published
```

Private repository access must be granted to individual GitHub accounts through **Settings → Collaborators → Add people**. Do not share GitHub passwords. Further instructions are in `deployment/PRIVATE_COLLABORATOR_REVIEW.md`.
