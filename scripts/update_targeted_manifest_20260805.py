...
def update_csv(root: Path, asset_root: Path) -> None:
    ...
    for figure, stem in TARGETS.items():
        ...
        row["App_location"] = f"outputs/{'final_publication_figures' if is_main else 'app_supplementary_figures'}/{stem}.png"
...
def update_commands(root: Path) -> None:
    ...
    output.insert(2, "Targeted taxonomy rule (2026-08-05): per sample, classified taxa strictly below 1% are grouped as `Other taxa (<1%)`; exactly 1% remains explicit; `Unclassified` is separate with exact percentage labels; no Top-N is applied.")
    path.write_text("\n".join(output) + "\n", encoding="utf-8")
