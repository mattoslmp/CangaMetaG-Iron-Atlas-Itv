...
Only exact, rank-aware taxonomic labels are replaced. OTU/count matrices,
sample identifiers, numeric values and the original missing-value literals
remain untouched. A packaged audit table provides an offline fallback, while
the reproducible refresh script can rebuild the mapping from the current NCBI
taxonomy dump for Phylum, Order, Family and Genus.
"""
...
def clean_taxonomy_label(value: object) -> str:
  """Normalize surrounding whitespace without collapsing scientific literals.

  The previous implementation converted every missing-value spelling to the
  single label ``Unclassified``. That changed the meaning of source tables and
  made it impossible to distinguish NA, N/A, Unknown, Unclassified and
  Unassigned. The harmonizer now preserves each literal exactly, apart from
  leading/trailing whitespace, and excludes such literals from rename maps.
  """
  return str(value if value is not None else "").strip()


def is_missing_taxonomy_literal(value: object) -> bool:
  return clean_taxonomy_label(value).casefold() in _MISSING
...
def load_name_updates(path: Path | str | None = None) -> pd.DataFrame:
  ...
  frame = frame[
    ...
    & ~frame["original_name"].map(is_missing_taxonomy_literal)
    & ~frame["current_name"].map(is_missing_taxonomy_literal)
  ].copy()
...
def update_mapping_by_rank(updates: pd.DataFrame | None = None) -> dict[str, dict[str, str]]:
  ...
  for _, row in frame.iterrows():
    ...
    if (
      rank in mapping
      and not is_missing_taxonomy_literal(old)
      and not is_missing_taxonomy_literal(new)
    ):
...
def harmonize_taxonomy_frame(
  ...
  """Return a label-updated copy while preserving geometry and measurements."""
  if frame is None:
    return pd.DataFrame()
  out = frame.copy()
...
def transfer_palette_names(
  ...
  for _, row in frame.iterrows():
    ...
    new = clean_taxonomy_label(row.get("current_name", ""))
    if (
      is_missing_taxonomy_literal(old)
      or is_missing_taxonomy_literal(new)
      or old == new
    ):
      continue
