...
replacements = [
  ...
  (
    ...
    '"Os MAGs 2, 5, 20, 32, 44, 47 e 49 são marcados explicitamente como sem clusters BGC identificados no conjunto antiSMASH final. Duplicatas enviadas para o mesmo MAG/bin foram resolvidas mantendo o arquivo selecionado no manifesto. O rótulo .repaired foi removido apenas dos nomes públicos; os arquivos internos de cada execução permanecem intactos.",\n       "MAGs 2, 5, 20, 32, 44, 47 and 49 are explicitly marked as having no BGC clusters identified in the final antiSMASH set. Duplicate uploads for the same MAG/bin were resolved by retaining the selected archive in the manifest. The .repaired label was removed only from public names; every internal run file remains intact."',
  ),
]
