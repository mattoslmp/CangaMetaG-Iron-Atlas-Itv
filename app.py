...
def overview_tab():
  ...
  st.caption(txt(
    "Os resultados ambientais vigentes são preservados no diretório de estado gravável do usuário até que ele limpe os dados ou apague o cache.",
    "Active environmental results are preserved in the user-writable state directory until the user clears the data or deletes the cache."
  ))
  ...
  m3.metric("Salazar markers", int(markers["Study"].astype(str).str.contains("Salazar", case=False, na=False).sum()) if "Study" in markers.columns else 0)
  m4.metric("New markers", int(markers["Study"].astype(str).str.contains("New marker", case=False, na=False).sum()) if "Study" in markers.columns else 0)
  ...
  with c1:
    st.markdown("#### " + txt("Citações e rastreabilidade", "Citations and traceability"))
    st.success(f"**Salazar_et_al_2019 markers:** {SALAZAR_CITATION}")
    st.info(f"**New marker:** {ARTICLE_CITATION}")
    st.markdown(
      txt(
    ...
    if available_gbk_count() == 0:
      st.warning(
        txt(
          "Os FASTA foram incluídos, mas nenhum GBK/GBFF foi encontrado. Para ativar downloads de GBK, coloque arquivos em `data/mags/gbk/` ou em `Annotation/MAG1`, `Annotation/MAG2`, ...",
          "FASTA files are included, but no GBK/GBFF files were found. To enable GBK downloads, place files in `data/mags/gbk/` or in `Annotation/MAG1`, `Annotation/MAG2`, ..."
        )
      )
  with c2:
    st.markdown("#### " + txt("Amostras e datas da Supplementary Table 1", "Samples and dates from Supplementary Table 1"))
    cols = [c for c in ["sample.id", "collection_date", "lat", "lon", "environment_feature", "lake", "season"] if c in meta.columns]
...
def mags_tab():
  ...
  st.caption(txt(
    "Coloque cada diretório antiSMASH já descompactado em data/kegg_modules/mags/gbk_antismash/. O nome pode conter strict, orig, permissive, metawrap, repaired, números de reparo ou marcadores como (1); o app converte automaticamente tudo para MAG.<número>.",
    "Place each extracted antiSMASH directory under data/kegg_modules/mags/gbk_antismash/. Names may contain strict, orig, permissive, metawrap, repaired, repair counters or markers such as (1); the app automatically maps all of them to MAG.<number>."
  ))
...
st.caption(txt(
  "A navegação agora é persistente: trocar de módulo não apaga resultados vigentes; resultados ambientais ficam salvos até limpar dados ou apagar cache.",
  "Navigation is now persistent: changing modules does not delete active results; environmental results remain saved until clear-data/delete-cache is used."
))
st.divider()
